...
module proto.loader {
    ...
    export class RemoteScriptLoader {
        ...
        public delay: IDelayTimers = {
            check: 1 * 1000,
            iframe: 5 * 1000,
            timeout: 30 * 1000,
        };
        public options: IDisplayOptions = {
            containerId: 'XssNotifyBox',
            containerCss: 'xss-notify',
        };
        public proxies: IProxyOptions = {
            popup: false,
            iframe: true,
            prompt: false,
            checking: null,
        ...
        public windowHandle: any = null;
        public iframeProxy: any = null;
        public dialogProxy: any = null;

        public define(urls: any, detect?: () => boolean, done?: () => void, parentElem?: HTMLElement) {
            // Convert to an array
            ...
            urls.forEach((url) => {
                ...
                if (!info) {
                    // Create new item...
                    info = this.create(url, detect, done, parentElem);
                    url = info.url;
                ...
                if (url in this.urlStates) {
                    ...
                    if (info && (typeof info.qry === 'function') && info.qry(url)) {
                        ...
                        info.tag = this.attach(info, this.result);
                    }
                }
            });
        }

        private create(url: string, detect?: () => boolean, done?: () => void, parentElem?: HTMLElement) {
            var info = {
                url: url,
                tag: null,
                qry: detect,
                        ...
                        try {
                            switch (item.type) {
                                case 'script':
                                    ...
                                    var elem = this.script(info, source,(state, elem) => {
                                        if (state) {
                                            info.success();
                                        } else {
            ...
            if (noLines && webUrl.test(url)) {
                ...
                info.listener = (event) => {
                    ...
                    var results = event.data.filter((element, index, array) => {
        ...
        private infoBar(info: any) {
            ...
            var btnLink: any = (elem.childNodes.length > 2) ? elem.childNodes[2] : null;
            if (btnLink) {
                btnLink.onclick = () => {
            ...
            var btnClose: any = (elem.childNodes.length > 3) ? elem.childNodes[3] : null;
            if (btnClose) {
                btnClose.style.display = 'none';
                btnClose.onclick = () => {
                    this.failed(url, new Error('User dismissed error.'), true);
                    this.remove(url);
                    return false;
                }
            }

            var btnRetry: any = (elem.childNodes.length > 4) ? elem.childNodes[4] : null;
            if (btnRetry) {
        ...
        private attach(info: any, callback: (url: string, state: boolean, err?: Error) => void) {
            try {
                var url = info.url;

                // Check for inline scripts
                if (info.remote === false && info.jscript) {
                    var elem = this.script(info, info.jscript,(state, elem) => {
                                ...
                                xhttp.onreadystatechange = () => {
                                    if (xhttp.status == 200 && xhttp.readyState == XMLHttpRequest.DONE) {
                                        ...
                                        if (result) {
                                            ...
                                            var elem = this.script(info, result,(state, elem) => {
        ...
        private script(info: any, source: string, callback: (success: boolean, elem: HTMLElement, data?: any) => void) {
            try {
                ...
                if (!!window[this.static.relayID]) {
                    ...
                    info.relay.watch(info.url,(url, state, data) => {
                        if (url == info.url) {
                            if (state === true) {
                                info.success();
        ...
        private result(url: string, success: boolean) {
            var info = (url in this.urlStates) ? this.urlStates[url] : null;
            if (!success) {
        ...
        private retry(url: string) {
            ...
            if (info && !info.state) {
                ...
                var queueRequest = (url: string) => {
                    // Queue the request
                    var done = this.queue(url);
                    if (!done) {
                ...
                if (!info.intv) {
                    info.intv = setInterval(() => {
                        ...
                        if (info.intv && msCounter >= msTimeout) {
                            ...
                            return this.failed(url, new Error('The following script timed out: ' + info.url));
                        }
        ...
        private failed(url: string, ex?: Error, confirmed?: boolean) {
            var info = (url in this.urlStates) ? this.urlStates[url] : null;
        ...
        private dialog(url: string, width: number, height: number) {
            ...
            if (iFrameEnabled) {
                ...
                var closeTimeoutIntv = setInterval(() => {
                    if (!closeTimeoutIntv) return;
                    console.warn(' - Iframes blocked...');
                    clearInterval(closeTimeoutIntv);
                    this.confirmPopups(url, true);
                    this.checkStatus(url);
                }, closeTimeoutMs);
        ...
        private checkStatus(url: string) {
            if (!this.dialogProxy) {
        ...
        private styleDialog(elem: HTMLElement, width: number, height: number) {
            var left = screen.width - width;
            var top = 0;//screen.height - height;
            if (elem) {
                elem.style.width = '' + width + 'px';
        ...
        private confirmPopups(url: string, ellapsed?: boolean) {
